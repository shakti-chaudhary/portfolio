...
export const zSlug = z.string().min(1, 'Slug is required').max(100).regex(/^[a-z0-9-]+$/, 'Slug may only contain lowercase letters, numbers, and hyphens')
...
export const zTagArray = z.array(zTag).min(1, 'At least one tag is required').max(10, 'Maximum 10 tags allowed')
